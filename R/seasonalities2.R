...
isInSchoolTerm <- function(day) {
    # School terms for 2020/21
  schoolTermDates <- c(
    # 2020
...
getTemperature <- function(day, amplitude = 6, mean_temp = 20, phase_shift = -1.5) {
...
generateSeasonalitiesData <- function(days) {
  dates <- as.Date("2020-01-01") + days - 1
  
  return( data.frame(
    days,
    publicHoliday = sapply(days, isPublicHoliday),
    schoolHoliday = sapply(days, isSchoolHoliday),
    weekdate      = weekdays(dates),
    month         = month(dates),
    year          = year(dates),
    lockdown      = sapply(days, underLockdown),
    temperature   = sapply(days, getTemperature)
  ))
}
